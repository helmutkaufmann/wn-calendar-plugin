...
class Plugin extends PluginBase
{
    ...
    public function registerNavigation()
    {
        return [
            "calendar" => [
                ...
                // Show this main menu if the user has ANY calendar-related permission.
                // This does NOT grant those permissions, it only controls visibility.
                "permissions" => [
                    "mercator.calendar.*",
                ],
    ...
    public function boot()
    {
        Event::listen("backend.menu.extendItems", function ($manager) {
            ...
            // Only add "entries" related items if the user can actually access entries
            if (!$user->hasAccess("mercator.calendar.access_entries")) {
                return;
            }
            ...
            foreach ($calendars as $calendar) {
                $dynamicItems["calendar-" . $calendar->slug] = [
                    ...
                    "icon"        => "icon-calendar-check-o",
                    "url"         => Backend::url(
                        "mercator/calendar/calendarentries?Filter[calendar]=" . $calendar->id
                    ),
                    "permissions" => ["mercator.calendar.access_entries"],
                ];
            }
    ...
    public function translateDate($date, $withYear = false)
    {
        ...
        $format = $withYear
            ? "LL"
            : (starts_with($locale, "en") ? "MMMM D" : "D MMMM");
